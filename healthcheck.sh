...
if [ -e /tmp/authorized_keys_checksum ] && [ -e /config/authorized_keys ]; then
    ...
    # /proc/net/tcp represents port 22 as 0016 and the ESTABLISHED TCP state as 01.
    SSHD_CONNECTION_COUNT=$(awk 'BEGIN { count = 0 } $2 ~ /:0016$/ && $4 == "01" { count++ } END { print count }' /proc/net/tcp /proc/net/tcp6 2>/dev/null)
